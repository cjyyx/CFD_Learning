...
$$
这是一个**差分方程**，这个方程说明，只要知道了$u_{n,i}$和$u_{n-1,i}$，就可以求出$u_{n+1,i}$。也就是说，只要知道了$u_{0,i}$和$u_{1,i}$，就可以依次求出之后的$u_{n,i}$。
...
应用达朗贝尔公式，可以得出$u(t,x)$的解析解为
$$
u(t,x)=\frac{v_0}{2a}\left(H(x+at-x_0)-H(x-at-x_0)\right)
$$
其中$H(x)$为阶跃函数。分析得解析解与数值解一致，故该求解偏微分方程方法有效。
## 参考资料
